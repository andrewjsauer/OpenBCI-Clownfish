...
void setup() {
  ...
  //attempt to open a serial port for toy "output" (i.e. Arduino)
  if (playWithToy) {
    try {
      verbosePrint("OpenBCI_GUI.pde: attempting to open serial/COM port for toy data output = " + serial_output_portName);
...
void initSystem() {
  ...
  if (playWithToy) {
    toy = new Toy(serial_output);
    dataProcessing_user = new DataProcessing_User(nchan, get_fs_Hz_safe(), toy);
  } else {
    dataProcessing_user = new DataProcessing_User(nchan, get_fs_Hz_safe(), null);
  }
